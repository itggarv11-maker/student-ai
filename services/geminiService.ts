...
const checkAndDeductTokens = (cost: number) => {
    // Secret access allows bypassing token checks entirely.
    if (localStorage.getItem('stubro_secret_access') === 'true') {
        return;
    }
...
export const fetchSampleTopicContent = async (topic: string): Promise<{ content: string; subject: Subject; classLevel: ClassLevel; }> => {
    checkAiService();
    // This feature is a demo, so we won't deduct tokens.
    // checkAndDeductTokens(2); 

    const prompt = `Provide a concise, comprehensive, and well-structured explanation for a student on the topic: "${topic}".
    The content should be detailed enough to be used with AI study tools (at least 300 words). Use headings, bold text, and bullet points.
    Also, identify the most appropriate Subject and Class Level (e.g., "Class 10") for this topic.
    Return the result as a JSON object with keys: "content", "subject", "classLevel".
    Example Subjects: ${Object.values(Subject).join(', ')}.
    Example Class Levels: ${CLASS_LEVELS.join(', ')}.
    `;
    
    const schema = {
        type: Type.OBJECT,
        properties: {
            content: { type: Type.STRING, description: "A detailed explanation of the topic, at least 300 words long." },
            subject: { type: Type.STRING, enum: Object.values(Subject) },
            classLevel: { type: Type.STRING, enum: CLASS_LEVELS },
        },
        required: ["content", "subject", "classLevel"]
    };
...
export const solveDoubtFromImage = async (imagePart: { inlineData: { mimeType: string; data: string; } }, userQuestion: string): Promise<string> => {
    checkAiService();
    checkAndDeductTokens(5);
    const textPart = { text: `${STUBRO_PERSONALITY_PROMPT}\n\nA student has uploaded an image and has the following doubt about it. Please provide a clear, step-by-step explanation to solve their doubt. Format your response clearly using headings, bold text, and bullet points where appropriate.\n\nStudent's question: "${userQuestion}"` };
...
export const generateScenesForTopic = async (topicContent: string, language: string, classLevel: ClassLevel): Promise<VisualExplanationScene[]> => {
    ...
    2. A detailed, vivid search query for a stock image that represents the scene.
            ...
            responseSchema: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        image_prompt: { type: Type.STRING, description: "A search query for a stock photo website." },
                    },
                    required: ["narration", "image_prompt"]
                }
            } 
    ...
    const generatedScenes = await Promise.allSettled(
        sceneBlueprints.map(async (blueprint: { narration: string, image_prompt: string }) => {
            const unsplashUrl = `https://source.unsplash.com/1280x720/?${encodeURIComponent(blueprint.image_prompt)}`;
            const imageResponse = await fetch(unsplashUrl);
            if (!imageResponse.ok) {
                throw new Error(`Failed to fetch image from Unsplash for prompt: ${blueprint.image_prompt}`);
            }
            const imageBlob = await imageResponse.blob();
            const imageBytes = await blobToBase64(imageBlob);
...
export const generateFullChapterSummaryVideo = async (sourceText: string, language: string, classLevel: ClassLevel): Promise<VisualExplanationScene[]> => {
    ...
    for (const blueprint of sceneBlueprints) {
        try {
            ...
            console.error("Skipping a failed image fetch for summary video:", imgErr);
        }
    }
